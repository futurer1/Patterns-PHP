...
* Вначале на основе конфигурации создаётся объект Создатель
* Затем Создатель может быть использован для генерации объектов различных реализаций, которые он умеет возвращать
*/
...
abstract class CommsManager {    //супер-класс Создателя, на основе которого строятся Создатели конкретных реализаций
    abstract function getApptEncoder();    //описывает перечень всех методов Создателей
}

class MegaCommsManager extends CommsManager {    //Создатель реализации 1 (возвращает объекты различных типов)
    public function getApptEncoder() {    //возвращает объект конкретной реализации 1
        return new MegaApptEncoder();   //объект является конкретной реализацией продукта, 
                                          //созданного на основе абстрактного класса продуктов
    }
}
...
class MegaApptEncoder extends ApptEncoder {    //конкретная реализация 1 продукта 1
    function encode() {
        return "Какой-то продукт типа MegaAppt\n";
    }
}

class BloggsApptEncoder extends ApptEncoder {    //конкретная реализация 1 продукта 2
    function encode() {
...
print $commsMgr->getApptEncoder()->encode();    //вызвали метод создателя (getApptEncoder), 
                                                //который возвращает объект конкретной реализации (MegaApptEncoder) и
                                                //выполнили метод encode() этого объекта
